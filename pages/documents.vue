<template>
  <div class="z-10 min-h-screen px-8 pt-8">
    ...
    <button class="text-light-gray" @click="connectMetaMask">Connecter Metamask</button>
    <p class="text-xs text-light-gray" v-if="account">Compte : {{ account }}</p>
      ...
      <div class="mt-6 mb-24 text-white">
        ...
        >
          <div
            ...
            :key="folder.id"
            class="text-center"
            @touchstart="touchStart($event, folder.name)"
          ...
          <form @submit.prevent="addFolder" class="flex flex-col items-center">
            <div>
              <IconAddFolder
                class="transition-transform transform cursor-pointer hover:scale-105"
            ...
            <div id="icon-add-folder" v-show="isButtonClicked">
              <div class="flex flex-col items-center">
                ...
                />
                <button
                  class="pt-2 transition-transform transform hover:scale-110"
                  type="submit"
                >
  ...
  methods: {
    async connectMetaMask() {
      if (this.$web3) {
        try {
          ...
          console.log("Connecté au compte :", this.account);
        } catch (error) {
    ...
    touchStart(event, folderName) {
      ...
      this.touchTimeout = setTimeout(
        () => this.showContextMenu(event, folderName),
        700
      );
    },
    ...
    async addFolder() {
      ...
      try {
        ...
        console.error(
          "Erreur lors de la soumission d'une nouvelle categorie:",
          error
        );
      }
    },
    ...
    async getAllFolders() {
      try {
        ...
        const response = await axios.get(`${BASE_URL}/api/Categories/user`, {
          headers,
        });
    ...
    async deleteFolder(folderName) {
      try {
        ...
        const response = await axios.delete(
          `${BASE_URL}/api/Categories/${folderName}`,
          { headers }
        );
